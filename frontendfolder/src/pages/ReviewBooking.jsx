...
export default function ReviewBooking() {
  ...
  useEffect(() => {
    ...
    // If new data comes from PickUp page, update and save it
    if (location.state) {
      const updatedData = { ...savedData, ...location.state };
  ...
  // Formated "yyyy-mm-dd" to "dd mon yyyy"
  ...
  const formattedPickUpDate = formatDate(bookingData.pickUpDate);
  const formattedReturnDate = formatDate(bookingData.returnDate);
  const formattedPickUpTime = formatTo12Hour(bookingData.pickUpTime);
  const formattedReturnTime = formatTo12Hour(bookingData.returnTime);
        ...
        <div className="mt-[13px] py-[32px] flex items-center">
          ...
          <div className="ml-[16px]">
            <p className="text-[17px] font-semibold">
              {bookingData.carName || "Car not selected"}
            </p>
